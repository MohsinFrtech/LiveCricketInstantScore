...
class TestRecentFragment:Fragment(),MatchItemClick, AdManagerListener {
    private val recentTestMatchesViewModel by lazy {
    ...
    override fun onCreateView(
        ...
        binding?.recentMatchesModel=recentTestMatchesViewModel
        adManagerClass = AdManager(requireContext(), requireActivity(), this)
        setTestDataForRecentMatches()
        return view
    }
    private fun setTestDataForRecentMatches() {
        recentTestMatchesViewModel.recentMatchesList.observe(viewLifecycleOwner, Observer {
            if (!it.isNullOrEmpty()) {
                binding?.noResult?.visibility(false)
                setRecyclerForTest(it)
            } else {
                binding?.noResult?.text = resources.getString(R.string.noRecenttest)
